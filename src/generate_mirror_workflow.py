...
def generate_mirror_workflow(project_name: str, repo_ssh: str, mirror_name: str):
    ...
    actions = [(["mkdir", f"{project_name}/{mirror_name}/.github"], "Folder already exist: " + f"{project_name}/{mirror_name}/.github"),
               (["mkdir", f"{project_name}/{mirror_name}/.github/workflows"], "Folder already exist: " + f"{project_name}/{mirror_name}/.github/workflows")]
    for action in actions:
        if subprocess.run(action[0]).returncode == 1:
            raise Exception(action[1])
    mirror_file = open(f"{project_name}/{mirror_name}/.github/workflows/verif_mirror.yml", "w")
